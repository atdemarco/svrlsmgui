function [success,handles] = RunAnalysis(hObject,eventdata,handles)
    if isempty(gcbo) || isa(gcbo,'matlab.ui.container.Menu'), handles.parameters.runfromgui = 0;
    else, handles.parameters.runfromgui = 1;
    end
    ...
    if parameters.method.mass_univariate
        ...
        msg = sprintf('Hyperparameters: C = %.2f, %s = %.2f, ε = %.2f, standardize = %s', hyperparms.cost, ...
            'σ', ...
            hyperparms.sigma, ...
            hyperparms.epsilon, ...
            myif(hyperparms.standardize,'true','false'));
        handles = UpdateProgress(handles,msg,1);
