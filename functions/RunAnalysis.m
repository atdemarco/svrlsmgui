...
if any(brain_nuisance_model_options)
    ...
    if doSaveCovMaps
        ...
        for c = 2 : size(variables.lesion_nuisance_model_betas,1) % since the 1st is constant... 
            % numel(variables.lesionNuisanceModelFields) 
            %curcovname = ['cov' num2str(c)];  %variables.lesionNuisanceModelFields{c};
            ...
            variables.vo.fname = fullfile(variables.output_folder.covariates,covfilename); % clusteredoutfname; % fullfile(variables.output_folder.clusterwise,'Thresholded by cluster size (betas).nii');
            svrlsmgui_write_vol(variables.vo, out_map);
        end
        %error('done saving covs')
    end
else
