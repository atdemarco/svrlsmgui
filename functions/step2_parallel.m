function [parameters,variables,thresholds] = step2_parallel(handles,parameters,variables,thresholds,all_perm_data)
    ...
    for j = 1 : njobs
        ...
        f(j) = parfeval(p,@parallel_step2_batch_fcn,2,job_indices,all_perm_data,total_cols,tail,ori_beta_vals,neg_thresh_index,pos_thresh_index,L,nperms,do_CFWER,outpath);
    end
...
function [alphas,betamapcutoffs] = parallel_step2_batch_fcn(this_job_cols,all_perm_data,total_cols,tail,ori_beta_vals,neg_thresh_index,pos_thresh_index,L,nperms,do_CFWER,outpath)
    alphas = nan(1,numel(this_job_cols)); % pre-allocate spac
    betamapcutoffs = nan(1,numel(this_job_cols)); % pre-allocate spac
    for jobcolind = 1:numel(this_job_cols) % 1 : length(variables.m_idx) % this is for each voxel in the brain, cutting across permutations
