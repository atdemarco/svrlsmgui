function build_cfwer_diagnostic_figure(cfwerdata)
   fig = figure('visible','on');
   a=subplot(1,3,1);
   [f,x] = ecdf(cfwerdata.cfwerinfo.p_of_requested_v);
   plot(a,x,f,'k--')
   hold on;
   [f,x]=ecdf(cfwerdata.cfwerinfo.pval_null_dist.pval);
   plot(a,x,f,'k-');
   legend({'null p dist','null p dist (unadj)'})
   title('ECDF of FWER P distribution')

   b=subplot(1,3,2);
   
% Empirical CDF of P(V_requested) and P(V_achieved)
   %figure;ecdf(cfwerdata.cfwerinfo.n_found_p_of_requested_v)
...
%   set(gca,'ylim',[.9 1])
    ...
    [f,x] = ksdensity(cfwerdata.cfwerinfo.pval_null_dist.pval,'kernel','epanechnikov','Support','positive','BoundaryCorrection','Reflection');
    plot(x,f,'k','parent',b);
    hold on;
    [f,x] = ksdensity(cfwerdata.cfwerinfo.p_of_requested_v,'kernel','epanechnikov','Support','positive','BoundaryCorrection','Reflection');
    plot(x,f,'k--','parent',b);
    hold on;
    line(cfwerdata.cfwerinfo.cfwer_single_pval_answer*ones(1,2),get(a,'ylim'),'color','r','parent',a)
    hold on;
    line(cfwerdata.cfwerinfo.cfwer_single_pval_answer*ones(1,2),get(a,'ylim'),'color','r','parent',b)
    ylabel('Density')
    xlabel('P-value')
    legend({'Null FWE distribution',sprintf('Whole-brain threshold (FWER = %.2f)',cfwerdata.cfwerinfo.cfwer_p_value),' p unadj'})
    title(sprintf('Null FWE P-distribution (v = %d mm^{3}, %d perms)',cfwerdata.cfwerinfo.v_value_in_mm3_actual_floor,555))
