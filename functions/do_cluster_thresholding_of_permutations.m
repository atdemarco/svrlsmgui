function do_cluster_thresholding_of_permutations(handles,parameters,variables,all_perm_data,thresholded)
    ...
    UpdateProgress(handles,'Applying voxelwise beta mask to null data and noting largest null clusters...',1);
    ...
    for f = 1 : parameters.PermNumVoxelwise % go through each frame of generated betas in the null data...
        ...
        thresholded = do_tail_thresholding(parameters,thresholded,handles.options,templatevol);

        % if requested, save raw voxel thresholded image for this permutation
        if parameters.SavePostVoxelwiseThresholdedPermutations 
            save_post_voxelwise_thresholded_permutation(parameters,variables,handles.options,thresholded)
        end
    ...
    save(fullfile(variables.output_folder.clusterwise,'Largest clusters.mat'),'all_max_cluster_sizes');
