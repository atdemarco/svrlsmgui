function x3mesh2(f,v, varargin)
% A simple function to convert a matlab mesh consisting of faces (f) and
% vertices (v) to x3dom html fle to allow viewing of the 3d mesh in a browser. 
...
%   outdir      : Directory to save outout file (default = pwd)
...
% output directory
p.addParamValue('outdir', pwd, @isstr); %@(x) exist(x,'dir') % < no we can make it:)
% sub heading
...
if ~exist(inps.outdir,'dir') % 'htmlfigs', 'dir')
    mkdir(inps.outdir) %mkdir('htmlfigs')
end

% open the file to write to
%fout = ['htmlfigs/' inps.name '.html'];
fout = fullfile(inps.outdir,[inps.name '.html']);
file1=fopen(fout,'w+');
